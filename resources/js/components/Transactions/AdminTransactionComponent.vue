    ...
    methods: {
            ...
            let result = await this.$swal({
                ...
                if (result.value) {
                    ...
                    if (response.data.data === 'OK') {
                        ...
                        this.$swal('Cancelled!','transaction has been cancelled.', 'success');
                    }
                }
        },
        async resetTimer() {
            let result = await this.$swal({
                ...
                if (result.value) {
                    let response = await this.$http.post(`/transactions/reset_timer/${this.transaction_id}`);
                    if (response.data.data === 'OK') {
                        this.$swal('Timer Reset!','transaction timer has been reset.', 'success');
        ...
        async resolveIssue() {
            let result = await this.$swal({
                ...
                if (result.value) {
                    let response = await this.$http.post(`/transactions/resolve_issue/${this.transaction_id}`);
                    if (response.data.data === 'OK') {
                        this.$swal('Issue Resolved!','transaction issue has been resolved.', 'success');
                        this.fetchTransaction();
                    }
                }
        }
